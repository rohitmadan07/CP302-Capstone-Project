...
void record_roi_stats(uint32_t cpu, CACHE *cache)
{
    for (uint32_t i=0; i<NUM_TYPES; i++) {
        ...
        cache->roi_my_stat[cpu][i] = cache->my_stat[cpu][i];
        //cache->roi_AMAT[cpu][i] = cache->AMAT[cpu][i];
    }
}
...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
    ...
    double Final_Hit_Ratio = (double)TOTAL_HIT/(double)TOTAL_ACCESS;

    cout << cache->NAME;
    cout << " TOTAL     ACCESS: " << setw(10) << TOTAL_ACCESS << "  HIT: " << setw(10) << TOTAL_HIT << "  MISS: " << setw(10) << TOTAL_MISS << endl;

    cout << cache->NAME;
    cout << " LOAD      ACCESS: " << setw(10) << cache->roi_access[cpu][0] << "  HIT: " << setw(10) << cache->roi_hit[cpu][0] << "  MISS: " << setw(10) << cache->roi_miss[cpu][0] << /*"  AMAT: " << */setw(10) << cache->roi_my_stat[cpu][0] << endl;

    cout << cache->NAME;
    cout << " RFO       ACCESS: " << setw(10) << cache->roi_access[cpu][1] << "  HIT: " << setw(10) << cache->roi_hit[cpu][1] << "  MISS: " << setw(10) << cache->roi_miss[cpu][1] << /*"  AMAT: " << */setw(10) << cache->roi_my_stat[cpu][1] << endl;

    cout << cache->NAME;
    cout << " PREFETCH  ACCESS: " << setw(10) << cache->roi_access[cpu][2] << "  HIT: " << setw(10) << cache->roi_hit[cpu][2] << "  MISS: " << setw(10) << cache->roi_miss[cpu][2] << /*"  AMAT: " << */setw(10) << cache->roi_my_stat[cpu][2] << endl;

    cout << cache->NAME;
    cout << " WRITEBACK ACCESS: " << setw(10) << cache->roi_access[cpu][3] << "  HIT: " << setw(10) << cache->roi_hit[cpu][3] << "  MISS: " << setw(10) << cache->roi_miss[cpu][3] << /*"  AMAT: " << */setw(10) << cache->roi_my_stat[cpu][3] << endl;
    if (cache->NAME == "LLC")
    {
        cout << cache->NAME;
        cout << " PREFETCH  REQUESTED: " << setw(10) << cache->pf_requested << "  ISSUED: " << setw(10) << cache->pf_issued;
        cout << "  USEFUL: " << setw(10) << cache->pf_useful << "  USELESS: " << setw(10) << cache->pf_useless << " pf_FILL: " << setw(10) << cache->pf_fill << endl;
        cout << cache->NAME;
        cout << " ON_TIME: " << setw(10) << cache->pf_usefull_on_time << "  LATE: " << setw(10) << cache->pf_late << "  LATE_CONSIDER: " << setw(10) << cache->pf_usefull_late << endl; //saurabh
        //cout << " RQ_Late_count " << setw(10) << cache->counter_0 << endl;
    }
    cout << cache->NAME;
    ...
    cout << "AVG READ REQUEST DELAY: " << (double)cache->READ_REQ_DELAY/((double)cache->OUR_RQ_COUNT) << endl;
...
void print_sim_stats(uint32_t cpu, CACHE *cache)
{
    ...
    cout << cache->NAME;
    cout << " LOAD      ACCESS: " << setw(10) << cache->sim_access[cpu][0] << "  HIT: " << setw(10) << cache->sim_hit[cpu][0] << "  MISS: " << setw(10) << cache->sim_miss[cpu][0] << /*"  AMAT: " << setw(10) << cache->AMAT[cpu][0] << */endl;

    cout << cache->NAME;
    cout << " RFO       ACCESS: " << setw(10) << cache->sim_access[cpu][1] << "  HIT: " << setw(10) << cache->sim_hit[cpu][1] << "  MISS: " << setw(10) << cache->sim_miss[cpu][1] << /*"  AMAT: " << setw(10) << cache->AMAT[cpu][1] << */endl;

    cout << cache->NAME;
    cout << " PREFETCH  ACCESS: " << setw(10) << cache->sim_access[cpu][2] << "  HIT: " << setw(10) << cache->sim_hit[cpu][2] << "  MISS: " << setw(10) << cache->sim_miss[cpu][2] << /*"  AMAT: " << setw(10) << cache->AMAT[cpu][2] << */endl;


    cout << cache->NAME;
    cout << " WRITEBACK ACCESS: " << setw(10) << cache->sim_access[cpu][3] << "  HIT: " << setw(10) << cache->sim_hit[cpu][3] << "  MISS: " << setw(10) << cache->sim_miss[cpu][3] << /*"  AMAT: " << setw(10) << cache->AMAT[cpu][3] << */endl;
...
void reset_cache_stats(uint32_t cpu, CACHE *cache)
{
    for (uint32_t i=0; i<NUM_TYPES; i++) {
        ...
        cache->my_stat[cpu][i] = 0;
        //cache->sim_hit_rate[cpu][i] = 0;
        //cache->AMAT[cpu][i] = 0;
    }
...
int main(int argc, char** argv)
{
    ...
    uncore.LLC.llc_prefetcher_initialize(0); //Nupur, Antara
    ...
    while (run_simulation) {
        ...
        for (int i=0; i<NUM_CPUS; i++) {
            ...
            /*
            if (show_heartbeat && (ooo_cpu[i].num_retired%1000000 == 0)) {
                cout << "Heartbeat CPU " << i << " instructions: " << ooo_cpu[i].num_retired << " cycles: " << current_core_cycle[i] << endl;
            }*/
    ...
    for (uint32_t i=0; i<NUM_CPUS; i++) {
        ...
        // Nupur, Antara - AMAT LLC
        for (uint32_t i=0; i<NUM_TYPES; i++) {
            TOTAL_ACCESS += uncore.LLC.roi_access[0][i];
            TOTAL_HIT += uncore.LLC.roi_hit[0][i];
        }
        double Final_Hit_Ratio = (double)TOTAL_HIT/(double)TOTAL_ACCESS;
        double DRAM_ACCESS_TIME = (uint32_t)((1.0 * tCAS_DRAM_NANOSECONDS * CPU_FREQ) / 1000);
        LLC_AMAT_FINAL = Final_Hit_Ratio*LLC_LATENCY +(1-Final_Hit_Ratio)*DRAM_ACCESS_TIME;
        cout << " AMAT LLC: " << LLC_AMAT_FINAL << " cycles" <<endl;

        // Nupur, Antara - AMAT L2C
        for (uint32_t i=0; i<NUM_TYPES; i++) {
            TOTAL_ACCESS += ooo_cpu[i].L2C.roi_access[0][i];
            TOTAL_HIT += ooo_cpu[i].L2C.roi_hit[0][i];
        }
        cout << " AMAT L2C: " << Final_Hit_Ratio*L2C_LATENCY +(1-Final_Hit_Ratio)*LLC_AMAT_FINAL << " cycles" <<endl;
        // --------------------------
